...
$sql = "SELECT DAYOFWEEK(due_date) as dow, SUM(TIMESTAMPDIFF(MINUTE, due_date, NOW())/60) as hours FROM tasks WHERE user_id=? AND completed=1 AND due_date >= NOW() - $interval GROUP BY dow";
...
$sql = "SELECT category, COUNT(*) as cnt FROM tasks WHERE user_id=? AND completed=1 AND due_date >= NOW() - $interval GROUP BY category";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
$sql = "SELECT topic, DATE_FORMAT(completed_at, '%Y-%u') as week, AVG(score/total*100) as avg_score FROM quizzes WHERE user_id=? AND completed_at >= NOW() - $interval GROUP BY topic, week ORDER BY week";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
$sql = "SELECT topic, AVG(score/total*100) as avg_score FROM quizzes WHERE user_id=? AND completed_at >= NOW() - $interval GROUP BY topic";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
$sql = "SELECT COUNT(*) as total, SUM(completed=1) as completed FROM tasks WHERE user_id=? AND due_date >= NOW() - $interval";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
$stmt->close();
// Quiz average
$sql = "SELECT AVG(score/total*100) as avg_score FROM quizzes WHERE user_id=? AND completed_at >= NOW() - $interval";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
// --- WEAK AREAS ---
$weakAreas = [];
$sql = "SELECT topic, AVG(score/total*100) as avg_score FROM quizzes WHERE user_id=? AND completed_at >= NOW() - $interval GROUP BY topic ORDER BY avg_score ASC LIMIT 3";
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $user_id);
$stmt->execute();
$res = $stmt->get_result();
